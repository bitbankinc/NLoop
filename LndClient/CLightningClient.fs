...
type CLightningClient (network: Network, address: Uri) =
  ...
  member private this.SendCommandAsync<'T>(cmd: string,
    ...
    backgroundTask {
      ...
      let result = JsonSerializer.DeserializeAsync<JsonElement>(networkStream, JsonSerializerOptions(), ct)
  ...
  interface INLoopLightningClient with
    member this.ConnectPeer(nodeId, host, ct) =
      task {
        ...
        let! resp = this.SendCommandAsync<obj>("connect")
        return failwith "todo"
      }
    member this.GetChannelInfo(channelId, ct) = failwith "todo"
    member this.GetDepositAddress(ct) = failwith "todo"
    member this.GetHodlInvoice(paymentHash, value, expiry, routeHints, memo, ct) = failwith "todo"
    member this.GetInfo(ct) =
      task {
        let ct = defaultArg ct CancellationToken.None
        let! resp = this.SendCommandAsync<GetInfoResponse>("getinfo", ct = ct)
        return resp |> box
      }

    member this.GetInvoice(paymentPreimage, amount, expiry, routeHint, memo, ct) = failwith "todo"
    member this.ListChannels(ct) = failwith "todo"
    member this.Offer(req, ct) = failwith "todo"
    member this.OpenChannel(request, ct) = failwith "todo"
    member this.QueryRoutes(nodeId, amount, maybeOutgoingChanId, ct) = failwith "todo"
    member this.SendPayment(req, ct) = failwith "todo"
    member this.SubscribeChannelChange(ct) = failwith "todo"
    member this.SubscribeSingleInvoice(invoiceHash, c) = failwith "todo"
    member this.TrackPayment(invoiceHash, c) = failwith "todo"
