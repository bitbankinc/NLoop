...
type RecentSwapFailureProjection(getOpts: GetOptions,
  ...
  let handleEvent (re: SerializedRecordedEvent) = unitTask {
    try
      ...
      | Ok r ->
        match r.Data with
        | Swap.Event.NewLoopOutAdded { LoopOut = o } when String.Equals(o.ChainName, getOpts().Network, StringComparison.OrdinalIgnoreCase) ->
          this.FailedLoopOutSwapState <-
            this.FailedLoopOutSwapState
          ...
          if this.FailedLoopOutSwapState.Count > InMemoryHistorySize then
            this.FailedLoopOutSwapState <-
              this.FailedLoopOutSwapState
              |> dropOldest
        | Swap.Event.NewLoopInAdded { LoopIn = i } when String.Equals(i.ChainName, getOpts().Network, StringComparison.OrdinalIgnoreCase)  ->
          i.LastHop
